...
ggraph(tidyverse_tbl, layout = 'nicely') + 
  geom_edge_link() + 
  #geom_node_point() +
  geom_node_label(aes(label = name), colour = "blue") +
  theme_ft_rc(grid="") +
...
<!-- The Hydrology Task Force aims to be the unique point where R users interested in analysing water data can find information on what tools are currently available to them.  -->
...
```{r countDownloads}
library(cranlogs)
## Some sort of downloads per year would be nice.

## select the last 5 years 
pkgcount <- cran_downloads(packages = hyd_packages, 
                           from = Sys.Date()-5*365, to = Sys.Date())
pkgcount %>% 
  group_by(package) %>% 
  summarise(downloads = sum(count)) %>% 
  arrange(desc(downloads)) 
# %>% View()
#   mutate(group = ifelse(downloads >= 10000, "Over 10000", "Under 10000")) %>% 
#   ggplot() +
#   geom_col(aes(x = package, y = downloads)) +
#   facet_wrap(~group, scales = "free")
# hydroTSM hydroGOF and rnoaa dominate the chart 
```
